...
function predict() 
    ...
    prob = ODEProblem(ODEsystem,h0,tspan_pred,p)
    solve(prob,Tsit5(),p=p,saveat=t_pred) 
end
...
cb = function()
    ...
    if iter % 1 == 0
       println("Iteration $iter || Loss = $(l)")
        ...
        cur_pred = cur[1,:]
        cur_vel = cur[2,:]
        pl = scatter(t,ode_data, xlims=tspan_pred, ylims=(-1.20,1.20),label="data",  annotations = (6,-0.9,Plots.text("iter = $iter")))
        pl =plot(pl,t_pred,cur_pred,linewidth=2,label="prediction",title = latexstring("u") * " vs " * latexstring("t") * " - SONODE")
