...
tspan = (0.0,12)                                # intervalo de tiempo
...
function construct_model(input_dim, hidden_dim, augment_dim)
    ...
    n_ode = NeuralODE(chain ,tspan, Tsit5(), saveat=t,reltol=1e-5,abstol=1e-5) 
...
cb = function()
    ...
    if iter % 50 == 0  
        ...
        cur = predict()
        pl = scatter(t,ode_data, xlims=tspan, ylims=(-1.20,1.20),label="data",legend=:topright, annotations = (6,-0.9,Plots.text("iter = $iter")))
        ...
        plot!(pl,t,cur[1,:],linewidth=2,label="prediction",title = latexstring("u") * " vs " *latexstring("t") * tit)
...
display(loss_list[iter_threshold-1])
display(loss_list[iter_threshold])
